...
abstract class Image
{
    ...
    protected function __construct(string $path)
    {
        ...
        if ($mimetype !== $this->getMimeType()) {
            throw  new ImageException('UNREADABLE_IMAGE');
        }
    ...
    public function copyResize(int $frameWidth, int $frameHeight, string $targetPath): void
    {
        ...
        if ($sourceRatio >= $frameRatio && $this->width > $frameWidth) {
            ...
            $targetWidht = $frameWidth;
            // Déterminer la hauteur en conservant les proportions.
            $targetHeight = (int)($targetWidht / $sourceRatio);
        ...
        elseif ($sourceRatio <= $frameRatio && $this->height > $frameHeight) {
            ...
            $targetWidht = (int)($targetHeight * $sourceRatio);
        ...
        $gdSource = $this->from();
        // Créer la ressource GdImage cible. Déclenche une exception si erreur.
        if (!$gdTarget = imagecreatetruecolor($targetWidht, $targetHeight))
            throw new ImageException(ImageException::TARGET_RESOURCE_CREATION_FAILED);
        // Redimensionner la ressource GdImage source vers la ressource GdImage cible. Déclenche une exception si erreur.
        if(!imagecopyresampled($gdTarget, $gdSource, 0,0,0,0,$targetWidht,$this->width,$this->height))
